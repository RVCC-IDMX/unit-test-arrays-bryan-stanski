...
function filterForLargeWords(arr, length) {
  const newArray = arr.filter((item) => item.length > length);
  return newArray;
}
...
function findTheFirstEvenNumber(arr) {
  const newArray = arr.find((element) => element % 2 === 0);
  return newArray;
}
...
function makeTheString(arr, separator) {
  const string = arr.join(separator);
  return string;
}
...
function makeMirrorArray(arr) {
  ...
  arr.reverse();
  arr.shift();
  const mirrorArray = newArray.concat(arr);
  return mirrorArray;
}
